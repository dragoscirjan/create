...
export const runAll = async (
  ...
  const tasks = new Listr(
    ...
    ],
    {
      ...
      concurrent: false,
      exitOnError: true,
    } as ListrOptions<CAListrContext>,
  );
...
export const prepareTaskByTag = (title: string, tag: string) => ({
  skip: (ctx: CAListrContext) =>
    ctx.tasks.filter((task: CATaskDescription) => task.tag === tag).length ===
    0,
  task: async (ctx: CAListrContext) => {
    // return new Promise((resolve) => setTimeout(resolve, 10000));
    const tasks = ctx.tasks
      .filter((task: CATaskDescription) => task.tag === tag)
      .map(prepareSubtask);
...
export const prepareListrTaskFromCommand =
  (task: CATaskDescription) => async (ctx: CAListrContext) => {
