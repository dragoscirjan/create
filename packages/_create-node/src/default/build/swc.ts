...
import { handleCompiledFile, writeEsmPackageJson } from "./babel.js";
import { buildSwcConfig } from "../build-tool/swc.js";
...
>): Promise<SwcOptions> {
  ...
  try {
    return await readFile(localPath, "utf-8").then((content) =>
      JSON.parse(content),
    );
  } catch (e) {
    logger?.warn(
      `error reading local ${localPath.replace(projectPath!, "")}, moving on with empty config`,
      e,
    );
  }
  return {};
