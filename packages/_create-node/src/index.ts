...
// import {BuildCommandOptions, CreateCommandOptions, QualityTool} from './types';
...
program
  ...
  // .option('--use-default-commands', 'Use the default build commands instead of `create-node build`')
  .action(
    async (
      projectPath: string,
      ...
      //   buildTool: options.buildTool || (options.language === 'ts' ? 'tsc' : 'babel'),
      //   logger,
...
//         ...json,
//         // TODO: coffee not supported yet
//         buildTool: options.buildTool || json.buildTool || (json.language === 'ts' ? 'tsc' : 'babel'),
//       };
//     });
