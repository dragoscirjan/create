...
describe('yarn', () => {
  ...
  describe('init', () => {
    ...
    it('install to call fs/spawn.spawn (saveDev)', async () => {
      ...
      expect(spawn.spawn).toHaveBeenCalledWith(['/bin/yarn', 'add', '-D', 'axios'], {cwd: options.projectPath});
    });
    ...
    it('install to call fs/spawn.spawn (save)', async () => {
      ...
      expect(spawn.spawn).toHaveBeenCalled();
      expect(spawn.spawn).toHaveBeenCalledWith(['/bin/yarn', 'add', 'axios'], {cwd: options.projectPath});
    });
    ...
    it('install to call fs/spawn.spawn (force)', async () => {
      ...
      expect(spawn.spawn).toHaveBeenCalled();
      expect(spawn.spawn).toHaveBeenCalledWith(['/bin/yarn', 'add', 'axios'], {
