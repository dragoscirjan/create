...
export default async function (options: CreateCommandOptions) {
  ...
  await readRepoFile(`../../static/${language}/rollup.config.js`, options).then((rollupConfig) =>
    writeFile('rollup.config.js', rollupConfig, options),
  );
  ...
  return updatePackageJson(options, (packageObject) => ({
    ...
    scripts: {
      ...
      ...(targets?.includes('node-cjs')
        ? {
            'build:node-cjs': 'cross-env BUILD_TARGET=node-cjs ROLLUP_BUILD=1 rollup -c',
          }
        : {}),
      ...(targets?.includes('node-esm') ? {'build:node-esm': 'cross-env BUILD_TARGET=node-esm rollup -c'} : {}),
    },
  }));
}
