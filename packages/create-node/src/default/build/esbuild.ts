...
export const esbuildTransformOptions = (options: BuildCommandOptions): esbuild.TransformOptions => {
  ...
  return {
    ...
    target: (target !== 'browser' ? 'es2020,node16' : 'es2020,chrome58,edge16,firefox57,safari11').split(','),
    ...
    .then(async (files) => {
      for (const file of files) {
        ...
        await readFile(file, 'utf-8')
          ...
          .then((result) => {
            handleCompiledFileAndMap(result?.code, result?.map, file.replace(/\.ts$/, '.js'), {
              logger,
              projectPath,
              target,
            });
