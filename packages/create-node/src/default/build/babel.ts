...
import {babelConfigBrowser, babelConfigCjs, babelConfigEsm, getTargetedBabelRcName} from '../build-tool/babel';
...
export async function readLocalBabelConfig({
  ...
  projectPath,
  target,
}: Pick<BuildCommandOptions, 'logger' | 'projectPath' | 'target'>): Promise<Record<string, unknown>> {
  ...
  try {
    ...
    logger?.warn(`require() unable to read ${localPath}, moving on with empty config`, e);
  }
  return {};
}
...
export async function buildBabelConfig({
  ...
  const codeConfig =
    target === 'node-cjs' ? babelConfigCjs : target === 'node-esm' ? babelConfigEsm : babelConfigBrowser;
      ...
      for (const file of files) {
        ...
        await readFile(file, 'utf-8')
          ...
          .catch((error) => {
            ...
            logger?.warn(`Error compiling ${file}:${error.loc.line}:${error.loc.column}`);
            errorCount++;
          });
      }
    });
