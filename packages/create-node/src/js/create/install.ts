...
const buildPackageList = <T extends CreateCommandOptions>({testFramework, buildTool}: T): string[] => {
  return [
    '@babel/cli',
    '@babel/core',
    '@babel/eslint-parser',
...
export default async function <T extends CreateCommandOptions>(options: T): Promise<void> {
  ...
  const {install: pmInstall} = await import(`../../util/package-manager/${packageManager}`);
